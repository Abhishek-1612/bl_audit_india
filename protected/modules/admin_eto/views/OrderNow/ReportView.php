...
$delivery_status = [0 => '',1 => 'Self Delivery' , 2 => 'Pickup By Buyer' , 3 => 'Delivery Courier Company'];
$payment_status = [0=>'', 1 => 'success', 2 => 'failure']; 
$payment_mode = [0=>'',1 => 'pay later', 2 => 'online paid order']; 
    ...
    if(is_array($arr))
    {
        if(count($indices)>0){
            ...
            if(key_exists($key,$arr)&& count($indices)>=0){
                $arr = $arr[$key];
                if(!is_array($arr) && $key == 'product_image' && $arr!=NULL){
                ...
                elseif(is_array($arr) && $key == 'product_image' && $arr!=NULL){
                    if(strpos($arr['image_100x100'],"\/"))$arr =  str_replace('\/','/',$arr['image_100x100']);
                    if(is_array($arr))$arr = $arr['image_100x100'];
...
<div id="reportView" style="margin-top:10px;padding-top:20px;">
        <table>
                <tr>
                    <td colspan='4'>
                        <div style = "width:100%;display:flex;justify-content:right;">
                            <button  style='background-color:#E92323;border-radius:50%;' id="btnProdModalClose">X</button>
                        </div>
                    </td>
                </tr>
                <tr>
                   <td colspan='4'>
                       <div style="display:flex;justify-content:space-between;">
                           ...
                           <div>                       
                                <h2>Order ID: <?php echo ($oid)?$oid:"";?></h2>
                            </div>
                       </div>
                   </td> 
                </tr>
                ...
                <tr>
                    ...
                    <td rowspan='2'>
                    <?php 
                    echo indexExists($data,['delivery_address','address_line_1'])." ".indexExists($data,['delivery_address','address_line_2'])." ".indexExists($data,['delivery_address','city'])." ".indexExists($data,['delivery_address','state'])." ".indexExists($data,['delivery_address','pincode']);
                    ?>   
                    </td>  
                    <th>Delivery Mode :</th> 
                    <td><input type='text' id='deliveryMode' name='deliveryMode' value="<?php echo $delivery_status[abs(indexExists($data,['delivery_mode']))];?>"></td> 
                </tr>
                <tr>
                ...
                <tr>
                    ...
                    <td><input type='text' id='paymentStatus' name='paymentStatus' value="<?php echo $payment_status[abs(indexExists($data,['payment_status']))];?>"></td> 

                </tr>
                <tr>
                    <th>Payment Mode :</th> 
                   <td><input type='text' id='paymentStatus' name='paymentStatus' value="<?php echo $payment_mode[abs(indexExists($data,['payment_mode']))];?>"></td> 
                ...
                <td colspan='4'>
                        <div style = "width:100%;display:flex;justify-content:center;">
                            <button  style='background-color:#E92323;border-radius:5%;' id="btnProdModalClose">Close</button>
                        </div>
                    </td>
            </table>
</div>
