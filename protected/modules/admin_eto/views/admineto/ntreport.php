    ...
    <td width = "90%" align="left" height="30" style="font-family:arial;font-size:14px;font-weight:bold;">
        ...
        if($totalRecords > 0){                               						
                 ...
                 for($i=1;$i<=$total_pg;$i++){                                                    
                    echo '<div><a class="pagina" href="http://'.$_SERVER['SERVER_NAME'].'/index.php?r=admin_eto/AdminEto/ntdashboard&mid=3443&action=export&total_records='.$totalRecords.'&start_date='.$start_date.'&end_date='.$end_date.'&vendor='.$vendor.'&source='.$source.'&employeeID='.$employeeID.'&start_time='.$start_time.'&end_time='.$end_time.'&tlid='.$tlid.'&in_flag='.$in_flag.'&start='.$nextStart.'&end='.$nextEnd.'"&mid=3443> Download Excel ( Page '.$i.' )</a></div><br>';
