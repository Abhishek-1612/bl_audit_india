...
{
	if (document.fcp_report.from_date.value == '')
	{
		alert('From date can not be blank');
		document.fcp_report.from_date.focus();
		return false;
	}
// 	if (document.fcp_report.to_date.value == '')
// 	{
// 		pickDate(false,to_date);
// 	}
	if (document.fcp_report.to_date.value != '')
	{

			var str1 = document.fcp_report.from_date.value;
			var str2 = document.fcp_report.to_date.value;
			if(str1 < str2)
			{
	...
	if (document.fcp_report.from_date.value != '')
	{
		if(document.fcp_report.to_date.value == '')
		{
				...
				monthMaxDaysLeap= [31, 29, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
				hideSelectTags = [];
				var from_date=document.fcp_report.from_date.value;
				...
				document.fcp_report.to_date.value=day+'-'+month+'-'+year;
...
function getDaysPerMonth(month, year)
{
	...
	*/
	...
	//alert(document.fcp_report.reporttype.value);
}
...
function LookupEmployee2(form,field,alphabet,empid)
{
    	var s = document.fcp_report.by_usr.value;
	s = s.replace(/(^\s*)|(\s*$)/gi,"");
	s = s.replace(/[ ]{2,}/gi," ");
	s = s.replace(/\n /,"\n");
	document.fcp_report.by_usr.value = s;

	if(document.fcp_report.by_usr.value =='')
	{
		alert('Please Enter Name To Search');
		document.fcp_report.by_usr.focus();
		return false;
	}
	if(document.fcp_report.by_usr.value.search(/^[0-9]+$/) != -1)
	{
		alert('Numeric Values Are Not Allowd');
		document.fcp_report.by_usr.value="";
		document.fcp_report.by_usr.value.focus();
		return false;
	}
	if(document.fcp_report.by_usr.value !='')
	{
		var iChars = "!~@#$%^&*()+=[]\\';,`/{}|\":<>?-_.";
		for(var i = 0; i < document.fcp_report.by_usr.value.length; i++)
		{
			if (iChars.indexOf(document.fcp_report.by_usr.value.charAt(i)) != -1)
			{
				alert('Name Should Not Contain Special Characters');
				document.fcp_report.by_usr.value='';
				document.fcp_report.by_usr.focus();
...
function process_data(ID,moduleID,likurl,pageID)
		{
			var actionID='';
			if(likurl)
			{
...
//  				alert (url);
			...
			{
// 				alert('else');
				url = '/admin_approval/personalise-mis-report.pl?Search=Search&action=Search&mid='+moduleID;
// 				alert(url);
			...
			//alert(trHtml);
					...
					http_request.onreadystatechange = function(){ alertContents1(http_request,actionID,pageID);};
					http_request.open('GET', url, true);
					http_request.send(null);
		}

		function alertContents1(http_request,actionID,pageID)
		{
			if (http_request.readyState == 4)
			{
				if (http_request.status == 200)
				{
					...
					if(my_result == 'OK')
					{
						document.getElementById('tr_'+actionID).innerHTML = '';
					}
					else
					{
						/*if(pageID == 1)
						{
							...
							document.getElementById('tabID').innerHTML = my_result;
						}
						else
						{*/
		...
		//  				}
			...
			else
			{
				if(actionID == 'page')
				{
					document.getElementById('tabID').innerHTML='<table style="border-collapse: collapse;width:80%" align="center" border="1" bordercolor="#bedaff" cellpadding="4" cellspacing="0" width="80%"><tr><TD ALIGN="CENTER" HEIGHT="100" STYLE="padding-top:0px;" colspan="7"> Processing.....<BR> <IMG SRC="/admin_glusr/img/loading2.gif" VSPACE="4" align="center"></TD></tr></table>';
				}
				else
				{
					document.getElementById('tr_'+actionID).innerHTML='<table style="border-collapse: collapse;width:80%" align="center" border="1" bordercolor="#bedaff" cellpadding="4" cellspacing="0" width="80%"><tr><TD ALIGN="CENTER" HEIGHT="100" STYLE="padding-top:0px;" colspan="7"> Processing.....<BR> <IMG SRC="/admin_glusr/img/loading2.gif" VSPACE="4" align="center"></TD></tr></table>';
				}
...
//  	}
	...
	var year = date[2];
			...
			var totaldays=getDaysPerMonth(mon,year);
			//change here for calender issue
			/*
			if(totaldays > Number(day))
			{
	 			var dayCounter=Number(day)+30;
...
function Check_glUserId() {
	var usr_id = document.fcp_report.usr_id.value;
	if(usr_id == '') {
		return Check_Date();
	}
	else 	if(usr_id.match(/^[0-9]+$/g)) {
			if (document.fcp_report.from_date.value == '' && document.fcp_report.to_date.value == '') {
				return true;
			}
			else {
				return Check_Date();
			...
			alert("Please enter numeric value only in Glusr ID");
			document.fcp_report.usr_id.focus();
			return false;
		}
