...
ORDER  BY BL_AUDIT_RESPONSE_ID, 
        ...
        $d1  = $d2 = $arr_q = $head = $d_final = array();
            ...
            $head[10]  = "Can be Approved";
            $head[11]  = "Can be Flagged";
            $head[12] = "Phone Etiquette Error";
            $head[13] = "Others";
            $head[14] = "Comments (if any)";
            $head[15] = "Score Including";
            $head[16] = "Raise Rebuttal";     
            $head[17] = "Wrong Deletion Error";       
            array_push($d1, $head);
            foreach ($dataArr as $sampleValue) {
                if ($p_res_id != $sampleValue['BL_AUDIT_RESPONSE_ID']) {
                    ...
                    $d1[$cnt][9]=$d1[$cnt][10]=$d1[$cnt][11]=$d1[$cnt][12]=$d1[$cnt][13]=$d1[$cnt][17]="-";
                ...
                if($sampleValue['OPT'] == "Wrong Deletion Disposition Selection"){
                    ...
                    $d1[$cnt][13]=$sampleValue['OPT'];                       
                }elseif($sampleValue['OPT'] == "Wrong Deletion"){
                    $d1[$cnt][17]=$sampleValue['OPT'];                       
                }
