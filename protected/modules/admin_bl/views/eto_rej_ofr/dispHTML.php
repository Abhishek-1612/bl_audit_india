...
function updateCall(){
	...
	glid = document.f1.report.value;
	url = "/index.php?r=admin_bl/Eto_rej_ofr/SaveCallUpdate&mid=3439";
	$.ajax({
			type: "POST",
			url: url,
			data:{radioValue:radioValue,glid:glid},
			//async: false,
			success: function(result){
				...
				if(result=='Yes'){
					...
					$('#failed4').css('display','inline-block');
				  // setTimeout(function(){ $('#failed2').fadeIn(); }, 6000);
				}
				
			},
			error: function() {
				$('#fsuccess4').css('display','inline-block');
				//  setTimeout(function(){ $('#failed2').fadeIn(); }, 6000);
				}
		});
}
function updateCaseClosure(arg,mcatid){
	$('.errorClss').css('display','none');
	// $('#failed2').css('display','none');
	glid = document.f1.report.value;
	var micro = $("#micro"+mcatid).val();
        var final = $("#final"+mcatid).val();
        var comment = $("#comment"+mcatid).val();
		if((micro == '') || (final == '')){
			alert('Please select a disposition!');
			return false;
		}
	url = "/index.php?r=admin_bl/Eto_rej_ofr/SaveCaseClosure&mid=3439";
	$.ajax({
			type: "POST",
			url: url,
			data:{micro:micro,glid:glid,final:final,comment:comment,mcatid:mcatid},
			//async: false,
			success: function(result){
				result = result.trim();
				$("input[type=Submit]").attr('disabled', false);
				if(result=='Yes'){
					$('#success1'+arg).css('display','inline-block');
					// setTimeout(function(){$('#success2').fadeIn();  }, 6000);
				}else{
					$('#failed1'+arg).css('display','inline-block');
				  // setTimeout(function(){ $('#failed2').fadeIn(); }, 6000);
				}
...
function initialMcats(){
	glid = document.f1.report.value;
        if(document.f1.report.value ==="") {
            alert("Please Enter GLUSER-ID");return false;
        }
        var start=document.f1.start_date.value;
	var end=document.f1.end_date.value;
        url = "/index.php?r=admin_bl/Eto_rej_ofr/InitialMcats&mid=3439&action=blni&glid="+glid+"&sdate="+start+"&tdate="+end;
	$.ajax({
			type: "POST",
			url: url,
			data:"",
			...
			success: function(result){
				$("input[type=Submit]").attr('disabled', false);
				if(result!=''){
					$("#initialMcats").html(result);
				}else{
				  $("#initialMcats").html('<span><center><font size = "3"><b>No Data Found.</b></font></center></span>');
				}
			},
			error: function() {
				$("input[type=Submit]").attr('disabled', false);
				$("#initialMcats").html('<span><center><font size = "3"><b>Something Went Wrong. Kindly Try again.</b></font></center></span>');
...
function updateNegativeMcats(arg){
    ...
    var mcatList = new Array();
		...
		if(Array.isArray(mcatList) && mcatList.length){

		}else{
			alert('Please select an MCAT to mark Negative!');
			return false;
		}
		//console.log(mcatList);
	  mcatList = mcatList.toString();
	   	   url = "/index.php?r=admin_bl/Eto_rej_ofr/SaveNegativeMcats&mid=3439";
					mapped_mcats='';
	$.ajax({
			type: "POST",
			url: url,
			...
			success: function(result){
				...
				if(result == 'Dont Mark'){
					...
					alert("Before marking as Negative, review Product Mapping for Mcats : "+mapped_mcats);
					return false;
				...
				if(result== 'Yes'){
					...
					if(arg == 1){
						...
						$('#failed3').css('display','inline-block');
					}
...
function getNegativeMcats(){
	glid = document.f1.report.value;
			url = "/index.php?r=admin_bl/Eto_rej_ofr/GetNegativeMcats&mid=3439&glid="+glid;
	$.ajax({
			type: "POST",
			url: url,
			data:"",
...
function relatedOffers(mcatid){
	...
	url = "/index.php?r=admin_bl/Eto_rej_ofr/OfferDetails1&mid=3439&mcatid="+mcatid+"&sdate="+sdate+"&tdate="+tdate+"&glid="+glid;
	$.ajax({
			type: "POST",
			url: url,
			data:"",
...
function load_detaildata(start_date,end_date,sbjVal,report){   
             var purl='';
            var post_data = {start_date:start_date, end_date:end_date,sbjVal:sbjVal,report:report}; 
            purl ="/index.php?r=admin_bl/Eto_rej_ofr/detaildata&mid=3439";
            $.ajax({
                 url: purl,
                 type: 'post',
                 data: post_data,
    ...
    <FORM ACTION="index.php?r=admin_bl/Eto_rej_ofr/Index&mid=3439" METHOD="post" NAME="f1">
    <table style="border-collapse: collapse;" width="100%" cellspacing="0" cellpadding="4" bordercolor="#bedaff" border="1">  <TR>
            <td colspan="2" bgcolor="#dff8ff" align="center"><font color=" #333399"><b>Rejection Report</b></font></td>			 	
      <td  ALIGN="right" rowspan="3">1. <a href="index.php?r=admin_bl/Eto_rej_ofr/Relevancyreport&mid=3439" target="_blank"> Relevancy Report </a><br>
                        2. <a href="index.php?r=admin_bl/Eto_rej_ofr/Summary&mid=3439" target="_blank"> Buy Leads Rejection Summary </a><br>
                        3. <a href="index.php?r=admin_bl/Eto_rej_ofr/Less90Days&mid=3439&action=less90" target="_blank"> <=90 Day Client </a><br>
        4. <a href="index.php?r=admin_bl/Eto_rej_ofr/Topmcatreport&mid=3439" target="_blank"> Top 10 Rejected Mcat Report</a><br>
        5. <a href="index.php?r=admin_eto/BLNIReport/Report&mid=3439" target="_blank">BL NI Audit Report</a></td>
        </TR>
      <tr>
      </tr>
      <?php 
      echo '<TR><TD width="30%">&nbsp;<SELECT NAME="detail" id="detail">
		...
		</SELECT>&nbsp;<INPUT NAME="report" TYPE="number" value="'.$sbjVal.'" style="width:130px;"> </TD>';?>      
	<TD width="40%"  align="left">&nbsp;Enter Date:&nbsp;
       <input name="start_date" type="text" VALUE="<?php echo $start_date; ?>" SIZE="13" onfocus="displayCalendar(document.f1.start_date,'dd-mm-yyyy',this,'','','from_date1')" onclick="displayCalendar(document.f1.start_date,'dd-mm-yyyy',this,'','','from_date1')" id="start_date" TYPE="text" readonly="readonly">
        &nbsp;to&nbsp;
        <input name="end_date" type="text" VALUE="<?php echo $end_date; ?>" SIZE="13" onfocus="displayCalendar(document.f1.end_date,'dd-mm-yyyy',this,'','','from_date1')" onclick="displayCalendar(document.f1.end_date,'dd-mm-yyyy',this,'','','from_date1')" id="end_date" TYPE="text" readonly="readonly">
	</TD>        
      </tr>
	  <input name="action" type="hidden" VALUE="" SIZE="13"  id="action" TYPE="text" readonly="readonly">
      <tr>	
        <TD align="right">
         <input style="color:white;background-color: #5A5A5A;cursor: pointer;padding: 5px;border-radius: 2px;font-family:arial;font-size:14px;font-weight:bold;" type="submit" value="Generate Report" ONCLICK="return validate(1)" name="genmis">
        </td>
        <TD align="left"><a type="submit" id="mcatButton" ONCLICK="return initialMcats()" 
        ...
        align="center">Wrong Product BLNI Report</a></TD>
      </TR>
    </TABLE></FORM>
<?php	
}
...
if($Frmdetail == 1)
 ...
 if(isset($rec['GLUSR_USR_COMPANYNAME']))
		...
		<b>Country:</b>&nbsp;'.$country.'<br>';
             echo '<div><B>Preferred Countries/States/Districts/Cities:</B> <a target="_blank" href="/index.php?r=admin_bl/Transaction_report/Prefcity&mid=3442&glid='.$glid.'">View Details</a></div>'
                     . '<div><B>Negative Countries/Cities:</B> <a target="_blank" href="/index.php?r=admin_bl/Transaction_report/Negativecity&mid=3442&glid='.$glid.'">View Details</a></div>'
                     . '<div><B>Preferred Categories only ( A & D rank categories):</B> <a target="_blank" href="/index.php?r=admin_bl/Transaction_report/Preferredcategory&mid=3442&glid='.$glid.'">View Details</a></div>'
                     . '<div><B>Negative Categories:</B> <a target="_blank" href="/index.php?r=admin_bl/Transaction_report/Negativecategory&mid=3442&glid='.$glid.'">View Details</a></div></td>';
		...
		echo '&nbsp;<a href="https://gladmin.intermesh.net/index.php?r=admin_bl/Setlocation_pref/Index&mid=3437&userid=441&empid='.$emp_id.'" target="_blank">Edit</a><br>'
                        . '<b>Dynamic Location Preference (2 months): </b>'.$loc_pref1.'<br><b>Dynamic Location Preference (1 year): </b>'.$loc_pref2.'<br></td>';
		echo '</tr>
                </tbody></TABLE>';
...
#by sarvesh
	...
	while ( $rec =  $sth_tot_rej->read()){
				$rec=array_change_key_case($rec, CASE_UPPER);
			  $all = $rec['INLEADS'] + $rec['FRLEADS'];
		          $inleads = $rec['INLEADS'];
			  $frleads = $rec['FRLEADS'];
			 $srcleads = $rec['SEARCH1'];
			 $myleads = $rec['MY'] - $rec['SEARCH1'];
			 $emailleads = $rec['EMAIL'];
			 $mobleads = $rec['MOBILE_SITE'];
                         $mobileapp=$rec['MOBILE_APP'];
			 $total_rejection = $all;
			 ...
			 echo '<TR bgcolor="#ffffff" >
					<TD ALIGN="LEFT"  class="ttext">Leads Rejected&nbsp;&nbsp;';
                          if($all > 0){
                            echo '<button onclick="load_detaildata('."'".$start_date."','".$end_date."','".$sbjVal."','".$report."'".');this.style.display = '."'".'none'."'".';">Show Detail</button>';
                        }
