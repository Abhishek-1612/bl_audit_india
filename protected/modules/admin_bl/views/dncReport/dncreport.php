...
<input name="frame_height" id="frame_height" value="" type="hidden">
	...
	<tr>
		...
		<td >&nbsp;Mod Id: </td>
		<td>&nbsp;<select name="modid" style="border-radius: 3px 3px 3px 3px;font-size: 100%;">
			<OPTION VALUE="ALL">ALL</OPTION>
			<?php 
				$modId=isset($_REQUEST['modid']) ? $_REQUEST['modid'] : '';
				foreach($rec as $k)
			{
				if(!empty($k['fk_gl_module_id']))
				{
				if($modId == $k['fk_gl_module_id'])
					{
						echo '<OPTION VALUE="'.$k['fk_gl_module_id'].'" SELECTED="SELECTED" >'.$k['fk_gl_module_id'].'</OPTION>';
					}
					else
					{
						echo '<OPTION VALUE="'.$k['fk_gl_module_id'].'"  >'.$k['fk_gl_module_id'].'</OPTION>';
					}
				   }		
