<?php 
error_reporting(0); 
class Bulkdata_upload extends CFormModel{
public $host_name;
...
if (isset($_REQUEST['act']) && $_REQUEST['act'] == 'update') { 
    ...
    $action='add';
        ...
        print <<<ae
    <html>
        <head><title>Bulk Big-Buyer organization Admin</title>
    ...
    <body>
        ...
        <form name="Search" method="post" enctype="multipart/form-data" action="/index.php?r=admin_bl/Bigbuyer/BulkBBAdd&act=$action&mid=$cookie_mid">
...
<td width=50%>
<div style="float:left"><B><a href="/temp_sample/admin_bl//bb_creation_format.xls">Download Sample Excel of Bulk Bigbuyer Addition</a></B></div><br/>
<div style="float:left"><B><a href="/temp_sample/admin_bl/bb_update_format.xls">Download Sample Excel of Bulk Bigbuyer Update</a></B></div><br/>
...
<div style="float:left"><B><a href="/temp_sample/admin_bl/bb_addemaildomain_format.xls">Download Sample Excel of Add Email Domains</a></B></div><br/>
...
hj;
    ...
    public function upload($empid,$cookie_mid) {
        ...
        $file_name .= "_" . $empid."_".$date."_bb.xls";          
        $split_empid  = '';
        $split_empid1 = '';
        $html         = '';
        $i            = 0;
...
gf;
                        ...
                        echo $total_rows = $data->sheets[0]['numRows'];                      
...
mn;
                        ...
                        $filename_out      = $upload_path ."bigbuyer_bulk_".$action.".xls";               
                        $BBFILE       = fopen($filename_out, "w");
                        if ($BBFILE === FALSE)
                            $remark = "Cannot open file [$filename_out] for writing";
                        if($action =='BB_ADD' || $action=='BB_UPDATE'){
                        ...
                        }else if($action =='BB_DELETE'){
                            if (fwrite($BBFILE, "IIL_BIG_BUYER_ORG_ID\t REMARK\t \n") === FALSE)
                                $remark = "Cannot write to file [$filename_out]";
                        }
                        $process_flag = 0;
                        $excel=$data->sheets[0];
                      // echo '<pre>';print_r($data->sheets[0]);echo '</pre>';die;
