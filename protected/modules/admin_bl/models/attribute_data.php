...
public function show_html($cookie_mid, $js_file) {
    ...
    <script language="javascript" src="admin_eto/common-scripts/bulk_bb_mgr.js?v=4"></script>
    </head>
...
<TR>
                ...
                <tr id ="tspbl_data"></tr>
                <tr>
                    <td align="center" colspan="2" bgcolor="#F0F9FF"><span id="trbtn">
ae;
        print <<<hj
...
<td width=50%>
<div style="float:left"><B><a href="/admin_eto/Sample_ISQ_Bulk_upload.xls">Download Sample Excel of ISQ Bulk Upload</a></B></div><br/>    
</td>
</tr>
    </table>
</td></tr>';
...
<div id="div_inst">
    ...
    <li>Only 1000 records will be process in one excel.</li>
...
gh;
 ...
 if (isset($_REQUEST['act']) && ($_REQUEST['act'] == 'add' || $_REQUEST['act'] == 'update')) {   
			...
			if (isset($data->sheets[0]['cells'][1][12])) {
                $heading_hash['SUP_PRIORITY'] = $data->sheets[0]['cells'][1][12];
            }  
			
			if (isset($data->sheets[0]['cells'][1][13])) {
                $heading_hash['OPT_SUP_PRIORITY'] = $data->sheets[0]['cells'][1][13];
            }
			...
			if ($heading_hash['SUP_PRIORITY'] != 'SUP_PRIORITY') {
                $heading_error .= "<BR>Column - K should be SUP_PRIORITY";
			}
			if ($heading_hash['OPT_SUP_PRIORITY'] != 'OPT_SUP_PRIORITY') {
                $heading_error .= "<BR>Column - K should be OPT_SUP_PRIORITY";
			}
}
...
mk;
        ...
        $EtoServiceModel = new EtoServiceModel(); 
                ...
                if ($match) {
                    ...
                    $upload_path = '';
                     if ($split_empid1 == $empid) {                        
                        ...
                        if ($total_rows > 1000) {
                            print <<<jh
<table style="border-collapse: collapse; width:98%" class="table_txt" align="center" border="1" bordercolor="#bedaff" cellpadding="4" cellspacing="0"><TR><TD bgcolor="#F0F9FF">Record Limit Exceed !! Only 1000 Records Are Allowed. You Have Entered <b>$total_rows</b> Records. Re-Upload Excel The File and Try Again.</TD></TR><TABLE>
jh;
...
fd;
                      ...
                      exit;
                        ...
                        $filename_out      = $upload_path ."isq_addition_bulk_".Yii::app()->session['empid'].".xls";               
                        ...
                        if($action =='BB_ADD'){
                            if (fwrite($BBFILE, "IM_CAT_SPEC_CATEGORY_ID\t IM_CAT_SPEC_CATEGORY_TYPE\t IM_SPEC_MASTER_DESC\t IM_SPEC_MASTER_FULL_DESC\t IM_SPEC_MASTER_BUYER_SELLER\t IM_SPEC_MASTER_TYPE\t IM_CAT_SPEC_PRIORITY\t IM_CAT_SPEC_STATUS\t IM_SPEC_OPTIONS_DESC\t IM_SPEC_OPTIONS_STATUS\t IM_SPEC_OPT_BUYER_SELLER\t INSERT_STATUS_IM_SPECIFICATION_MASTER\t INSERT_STATUS_IM_CAT_SPECIFICATION\t INSERT_STATUS_IM_SPECIFICATION_OPTIONS\t INSERT_STATUS_SUP_PRIORITY\t INSERT_STATUS_OPT_SUP_PRIORITY\n") === FALSE)
                        ...
                        $Arr_SUP_PRIO = $Arr_OPT_SUP_PRIOR = array();
                                               
                        $n=-1;$m=0;
                        foreach (range(2, $total_rows) as $i) {                            
                            $excel_row=array();
                            $finalmsg='';
                            $iserror_id = 1;$iserror_dlr = 1;
                        if($action =='BB_ADD')
						{
                            ...
                            $excel_row['SUP_PRIORITY']="";     
                            $excel_row['OPT_SUP_PRIORITY']="";   
							
                            if (isset($data->sheets[0]['cells'][$i][1])) {                                
                                $excel_row['IM_CAT_SPEC_CATEGORY_ID']=trim($data->sheets[0]['cells'][$i][1]);                                                                   
                            }
                            if (isset($data->sheets[0]['cells'][$i][2])) {
                               $excel_row['IM_CAT_SPEC_CATEGORY_TYPE']=trim($data->sheets[0]['cells'][$i][2]);
                            }
                            if (isset($data->sheets[0]['cells'][$i][3])) {
                               $excel_row['IM_SPEC_MASTER_DESC']=trim($data->sheets[0]['cells'][$i][3]);
                            }
                            if (isset($data->sheets[0]['cells'][$i][4])) {
                               $excel_row['IM_SPEC_MASTER_FULL_DESC']=trim($data->sheets[0]['cells'][$i][4]);
                            } 
                            
                            if (isset($data->sheets[0]['cells'][$i][5])) {
                               $excel_row['IM_SPEC_MASTER_BUYER_SELLER']=trim($data->sheets[0]['cells'][$i][5]);
                            }
                            if (isset($data->sheets[0]['cells'][$i][6])) {
                               $excel_row['IM_SPEC_MASTER_TYPE']=trim($data->sheets[0]['cells'][$i][6]);
                            } 
                            if (isset($data->sheets[0]['cells'][$i][7])) {                                
                                    $excel_row['IM_CAT_SPEC_PRIORITY']=trim($data->sheets[0]['cells'][$i][7]);                                   
                            }
                            if (isset($data->sheets[0]['cells'][$i][8])) {
                               $excel_row['IM_CAT_SPEC_STATUS']=trim($data->sheets[0]['cells'][$i][8]);
                            } 
                            if (isset($data->sheets[0]['cells'][$i][9])) {
                               $excel_row['IM_SPEC_OPTIONS_DESC']=trim($data->sheets[0]['cells'][$i][9]);
                            } 
                            if (isset($data->sheets[0]['cells'][$i][10])) {
                               $excel_row['IM_SPEC_OPTIONS_STATUS']=trim($data->sheets[0]['cells'][$i][10]);
                            } 
                            if (isset($data->sheets[0]['cells'][$i][11])) {
                               $excel_row['IM_SPEC_OPT_BUYER_SELLER']=trim($data->sheets[0]['cells'][$i][11]);
                            }
							if (isset($data->sheets[0]['cells'][$i][12])) {
                               $excel_row['SUP_PRIORITY']=trim($data->sheets[0]['cells'][$i][12]);
                            }
							if (isset($data->sheets[0]['cells'][$i][13])) {
                               $excel_row['OPT_SUP_PRIORITY']=trim($data->sheets[0]['cells'][$i][13]);
                            }
                           

                            if(is_numeric($excel_row['IM_CAT_SPEC_CATEGORY_ID']) &&  $excel_row['IM_CAT_SPEC_CATEGORY_ID']>0 && $excel_row['IM_CAT_SPEC_CATEGORY_TYPE'] >2 && $excel_row['IM_SPEC_MASTER_DESC']<>'' && $excel_row['IM_SPEC_MASTER_BUYER_SELLER']<>'' && $excel_row['IM_SPEC_MASTER_TYPE']<>'' && $excel_row['IM_CAT_SPEC_PRIORITY']<>'' && $excel_row['IM_SPEC_OPTIONS_DESC']<>'' && $excel_row['IM_SPEC_OPTIONS_STATUS']<>'' && $excel_row['IM_SPEC_OPT_BUYER_SELLER']<>'' && is_numeric($excel_row['SUP_PRIORITY'])& is_numeric($excel_row['OPT_SUP_PRIORITY']))
                                {                            
									...
									if($CATEGORY_ID !=$excel_row['IM_CAT_SPEC_CATEGORY_ID'])
									{
										...
										$finalDataArr[$n][$m]['SUP_PRIORITY']=$excel_row['SUP_PRIORITY'];
										$finalDataArr[$n][$m]['OPT_SUP_PRIORITY']=$excel_row['OPT_SUP_PRIORITY'];
								...
								if($IM_SPEC_OPTIONS_DESC!=utf8_decode($IM_SPEC_OPTIONS_DESC))
								{
                                    ...
                                    $INSERT_STATUS_SUP_PRIORITY=   "\"" . $excel_row['SUP_PRIORITY']. "\"";  
                                    $INSERT_STATUS_OPT_SUP_PRIORITY=   "\"" . $excel_row['OPT_SUP_PRIORITY']. "\"";  
                                    //$REMARK=   "\"" . $finalmsg. "\""; 
					...
					{
						${"Arr_CATEGORY_ID".$j}=${"Arr_CATEGORY_TYPE".$j}=${"Arr_MASTER_DESC".$j}=${"Arr_MASTER_FULL_DESC".$j}=${"Arr_MASTER_BUYER_SELLER".$j}=${"Arr_MASTER_TYPE".$j}=${"Arr_SPEC_PRIORITY".$j}=${"Arr_SPEC_STATUS".$j}=${"Arr_MASTER_DESC_UNIQ".$j}=${"Arr_MASTER_TYPE_UNIQ".$j}= ${"Arr_SUP_PRIO".$j} = ${"Arr_OPT_SUP_PRIOR".$j} = array();
						...
						for($i=0;$i<count($finalDataArr[$j]);$i++)
						{
							if(($cat_id ==$finalDataArr[$j][$i]['IM_CAT_SPEC_CATEGORY_ID']) && ($master_desc ==$finalDataArr[$j][$i]['IM_SPEC_MASTER_DESC']))
							{
								...
								${"Arr_OPT_SUP_PRIOR"}[$k][$z]=$finalDataArr[$j][$i]['OPT_SUP_PRIORITY'];
							}
							else{
								$z=0;
								$k++;
								...
								array_push(${"Arr_SUP_PRIO".$j},$finalDataArr[$j][$i]['SUP_PRIORITY']);
						...
						$Arr_SUP_PRIO=array_merge($Arr_SUP_PRIO,${"Arr_SUP_PRIO".$j});
						$Arr_SPEC_STATUS=array_merge($Arr_SPEC_STATUS,${"Arr_SPEC_STATUS".$j});
					}
					
					$screen='GLADMIN(Bulk Upload)';
					
					$x = array($empName,'',$histip,$Arr_MASTER_DESC,$Arr_MASTER_FULL_DESC,$Arr_MASTER_TYPE,$Arr_MASTER_BUYER_SELLER,$update,$screen);
					echo '<pre>';
					
print_r($x);
echo "#########";
$y = array($empName,$histip,$QuesMasterId,$Arr_CATEGORY_ID,$Arr_CATEGORY_TYPE,$Arr_SPEC_PRIORITY,$Arr_SPEC_STATUS,$update,$screen,$Arr_SUP_PRIO);
					echo '<pre>';
...
die;
					...
					{ 
						$QuesMasterId=$EtoServiceModel->ADDISQ_IM_SPECIFICATION_MASTER($empName,'',$histip,$Arr_MASTER_DESC,$Arr_MASTER_FULL_DESC,$Arr_MASTER_TYPE,$Arr_MASTER_BUYER_SELLER,$update,$screen);
          
						if(!empty($QuesMasterId))
						{
							$msg=$EtoServiceModel->ADDISQ_IM_CAT_SPECIFICATION($empName,$histip,$QuesMasterId,$Arr_CATEGORY_ID,$Arr_CATEGORY_TYPE,$Arr_SPEC_PRIORITY,$Arr_SPEC_STATUS,$update,$screen,$Arr_SUP_PRIO);
						}
						...
						if($msg =='SUCCESS')
						{
							...
							for($i=0;$i<count($Arr_OPTIONS_DESC);$i++)
							{
								...
								foreach($Arr_OPT_SUP_PRIOR[$i] as $x)
								{
									array_push($OPTIONSPrioArray,$x);
								}			  
							}
												
							$msg2=$EtoServiceModel->ADDISQ_IM_SPECIFICATION_OPTIONS($empName,'',$histip,$QuesMasterId1,$OPTIONS_DESCArray,$OPTStatus,$OPTIONS_ForArray,$QuestypeArray,$update,$screen,$OPTIONSPrioArray);
...
public function mailsent($content,$filename_out,$process_time,$action){  
                        ...
                        $to       = "$empname<$empmail>";
                            ...
                            $subject='ISQ Bulk Creation [Start Time : '.$process_time . " End Time : " . date("F j, Y, g:i a") ."]";                        
